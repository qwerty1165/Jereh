...
//		fit:true,
		...
		pageNumber:<%=session.getAttribute("purchaseOrderPageNo")%>
	});
	...
	$("#window").dialog({
		...
		buttons:[{
			text:'新 增',handler:function(){
				if(opt == 1){
					$("#windowForm").attr("action",
							"/jereh/purchase/AddPurchaseOrderServlet?purchaseOrderPageNo=" + 
									getCurrPageNo());
					$("#windowForm").get(0).submit();
				}
				if(opt == 3){
					$("#windowForm").attr("action",
							"/jereh/purchase/AddPurchaseOrderDetailServlet?purchaseOrderPageNo=" + 
									getCurrPageNo());
					$("#windowForm").get(0).submit();
				}
			}
			...
			text:'保 存',handler:function(){
				if(opt == 2){
					$("#windowForm").attr("action",
							"/jereh/purchase/UpdatePurchaseOrderServlet?purchaseOrderPageNo=" + 
									getCurrPageNo());
					$("#windowForm").get(0).submit();
				}
			}
...
function rst(){
	...
	$("input[name='fax']").val("");
	$("input[name='trans']").val("");
	$("input[name='deliveryDate']").val("");
	$("input[name='businesser']").val("");
...
function detailRst(){
	...
	$("input[name='ocode']").val("");
	$("input[name='xcode']").val("");
	$("input[name='pcode']").val("");
	$("input[name='pdNums']").val("");
	$("input[name='price']").val("");
	$("input[name='rkState']").val("");
	$("input[name='rkNums']").val("");
	$("input[name='remarks']").val("");
}
function rstTool(){
	$("input[name='purchaseOrderCode']").val("");
	$("#beforeDateTool").datebox("setValue","");
	$("#afterDateTool").datebox("setValue","");
	$("input[name='purchaseOrderSupplierCode']").val("");
}

/*得到当前页码*/
function getCurrPageNo(){
	return $("#purchaseOrderList").datagrid("getPager").data("pagination").options.pageNumber;  
}

//删除单个
function del(idx){
	var row = $("#purchaseOrderList").datagrid("getData").rows[idx];
	$.messager.confirm("删除提醒","确实执行删除操作吗？",function(r){
		if(r){
			window.location.href = "/jereh/purchase/DeletePurchaseOrderServlet?purchaseOrderPageNo=" + 
...
function deleteBatch(){
	var rows = $("#purchaseOrderList").datagrid("getSelections");//得到被select的一个数组
	if(rows.length == 0)
		...
		$.messager.confirm("删除确认","确实要删除记录吗？",function(r){
			if(r){//如果点击了确定则执行
				...
				window.location.href = "/jereh/purchase/DeleteBatchPurchaseOrderServlet?purchaseOrderPageNo=" + 
...
function update(idx){
	...
	var row = $("#purchaseOrderList").datagrid("getData").rows[idx];
	$.ajax({
		url:'/jereh/purchase/PurchaseOrderServlet?code=' + row.code,
		//async:false,//这种情况很少见；//async:true,//默认，异步，并行；
		dataType:'json',
		error:function(){},
		success:function(data){
			$("input[name='code']").val(data[0].code);
			$("input[name='orderDate']").val(data[0].orderDate);
			...
			$("select[name='trans']").find("option").eq(data[0].trans).attr("selected","selected");//不容易
			$("input[name='deliveryDate']").val(data[0].deliveryDate);
			$("input[name='businesser']").val(data[0].businesser);
...
function find(){
	$("#findForm").attr("action",
			"/jereh/purchase/FindPurchaseOrderServlet?purchaseOrderPageNo=" + 
					getCurrPageNo());
...
//		fitColumns:true,
		...
		url:'/jereh/purchase/GetPurchaseOrderDetailServlet?ocode=' + row.code,
		columns:[[
			{field:'dcode',title:'明细编号',width:150},
			{field:'ocode',title:'订单编号',width:150},
			{field:'xcode',title:'询价编号',width:150},
			{field:'pcode',title:'配件编号',width:150},
			{field:'nums',title:'数量',width:150},
			{field:'rkState',title:'配件是否入库',width:150},
			{field:'rkNums',title:'已入库数量',width:150},
...
function excelOut(){
	$.messager.confirm("导出excel","确实执行导出excel操作吗？",function(r){
		if(r){
			window.location.href = "/jereh/purchase/PurchaseOrderExcelServlet?purchaseOrderPageNo=" + 
					getCurrPageNo();
		}
	});
}
...
function wordOut(idx){
	...
	$.ajax({
		...
		success:function(data){
			...
			$("input[name='orderDate2']").val(data[0].orderDate);
			...
			var transObj = $("select[name='trans']").find("option").eq(data[0].trans).text();
			if(transObj == '-请选择-')
				transObj = '';
			$("input[name='trans2']").val(transObj);
		...
		success:function(data){
			...
			for(var i = 0; i < data.length; i++){
				...
				trObj.append(
					...
					'<td class="detailTd" style="width:130px;">' + data[i].ocode + "</td>" +
					'<td class="detailTd">' + data[i].xcode + "</td>" +
...
function detailDel(idx){
	...
	$.messager.confirm("删除提醒","确实执行删除操作吗？",function(r){
		if(r){
			window.location.href = "/jereh/purchase/DeletePurchaseOrderDetailServlet?purchaseOrderPageNo=" + 
					getCurrPageNo() + "&dcode=" + row.dcode + "&ocode=" + row.ocode;
		}
	});
}
...
<div id="tool">
	<form id="findForm" action="" method="post">
		<b>检索条件：</b>询价编号：<input type="text" name="purchaseOrderCode" style="width:130px;" />&nbsp;开始日期：<input 
				type="text" class="easyui-datebox" id="beforeDateTool" name="purchaseOrderBeforeDate" 
				style="width:130px;" />&nbsp;结束日期：<input type="text" class="easyui-datebox" 
				id="afterDateTool" name="purchaseOrderAfterDate" style="width:130px;" />&nbsp;供应商名：<input 
				type="text" name="purchaseOrderSupplierCode" style="width:130px;" /><input type="button" 
...
<div id="window" style="padding:10px;">
	<form id="windowForm" action="" method="post">
		<table id="order" border="1" cellspacing="0" style="border-collapse:collapse;">	
            <tr>
                <td class="label">订单编号：</td>
                <td class="in"><input type="text" name="code" readonly="readonly" /></td>
                <td class="label">订单日期：</td>
                <td class="in"><input type="text" name="orderDate" readonly="readonly" /></td>
            </tr>
            <tr>
            ...
            <tr>
            	...
            	<td class="label">交货日期：</td><td><input type="text" name="deliveryDate" /></td>
            </tr>
            <tr>
            	<td class="label">业务人员：</td><td><input type="text" name="businesser" /></td>
    	...
    	<br/>
    	<table id="orderDetail" border="1" cellspacing="0" style="border-collapse:collapse;">	
    		<tr>
                <td class="label">明细编号：</td>
                <td class="in"><input type="text" name="dcode" /></td>
                <td class="label">订单编号：</td>
                <td class="in"><input type="text" name="ocode" readonly="readonly" /></td>
            </tr>
            <tr>
                <td class="label">询价单号：</td><td><input type="text" name="xcode" /></td>
            ...
            <tr>
            	<td class="label">配件是否入库：</td><td><input type="text" name="rkState" /></td>
            	<td class="label">已入库数量：</td><td><input type="text" name="rkNums" /></td>
            </tr>
            <tr>
            	<td class="label">备注：</td>
            	<td colspan="3"><input type="text" name="remarks" style="width:600px;" /></td>
	...
	<form id="wordForm" action="" method="post">
		<table border="1" cellspacing="0" style="border-collapse:collapse;">	
	    	<tr>
	        	...
	        	<td class="label">订单日期：</td><td class="in"><input type="text" name="orderDate2" readonly="readonly" /></td>
	        </tr>
	        <tr>
	        ...
	        <tr>
	        	<td class="label">运输方式：</td><td class="in"><input type="text" name="trans2" readonly="readonly" /></td>
	        	<td class="label">交货日期：</td><td class="in"><input type="text" name="addDate2" readonly="readonly" /></td>
	        </tr>
	        <tr>
