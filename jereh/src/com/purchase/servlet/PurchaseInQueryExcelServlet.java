...
public class PurchaseInQueryExcelServlet extends HttpServlet {
	 ...
	 */
	public PurchaseInQueryExcelServlet() {
		super();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(pageSize == null || pageSize.equals("") || pageSize.equals("null"))
			pageSize = "5";
		PageBean pb = pService.getPurchaseInQuery(code, beforeDate, afterDate, supplierCode, 
				Integer.parseInt(pageNo), Integer.parseInt(pageSize));//
		List<PurchaseInQuery> pList = pb.getData(); 
		
		//EXCEL部分
		String t = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());//设置日期格式
		File file = new File("f://询价单据报表" + t + ".xls");
        ...
        try {
			...
			//表格列名
			sheet.addCell(new Label(0,1,"询价编号",cellFormate));
			sheet.addCell(new Label(1,1,"询价日期",cellFormate));
